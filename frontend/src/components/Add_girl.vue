<template>
  <tbody>
    <tr>
      <th class="text-center table_title" colspan="2">女の子追加</th>
    </tr>
    <tr>
      <th>公開設定</th>
        <td>
    ...
    <tr>
      ...
      <td>
        <input type="text" class="w-100" v-model="data.castName">
        <p v-if="isDuplicateName" class="cautionary_note text-danger mb-0">既に登録されている名前です。他の名前に変更してください。</p>
      </td>
    </tr>
    <tr>
      <th>入店日</th>
    ...
    <tr>
      <th>
        各種フラグ
        <p class="fw-normal text-danger cautionary_note" style="margin-bottom: 0;">※最大6つ迄のご登録となります（ピックアップ・プライズ除く）7個以上ご登録されるとレイアウトが崩れますのでご注意下さい。</p>
    ...
    <tr>
      <th>お店からのコメント</th>
      <td>
        <jodit-editor v-model="data.shopComment" :editor-options="joditOptions"></jodit-editor>
      </td>
    </tr>
    <tr>
      <th>女の子からのコメント</th>
      <td>
        <jodit-editor v-model="data.girlComment" :editor-options="joditOptions"></jodit-editor>
      </td>
    </tr>
    <tr>
      <th>セールスポイント</th>
      ...
      <td>
        ...
        <ul class="list-unstyled d-flex flex-wrap" style="margin: 0;">
          <li v-for="point in fjSellingPoints2" :key="point.value" class="selling_point">
            <label>
              <input type="checkbox" name="data.fj_selling_points2" @change="checkboxMaxSelection(2, 'data.fj_selling_points2')" v-model="data.fj_selling_points" :value="point">{{ point.label }}
            </label>
          </li>
        </ul>
        ...
        <ul class="list-unstyled d-flex flex-wrap" style="margin: 0;">
          <li v-for="point in fjSellingPoints3" :key="point.value" class="selling_point">
            <label>
              <input type="checkbox" name="data.fj_selling_points3" @change="checkboxMaxSelection(1, 'data.fj_selling_points3')" v-model="data.fj_selling_points" :value="point">{{ point.label }}
            </label>
          </li>
        </ul>
        ...
        <ul class="list-unstyled d-flex flex-wrap" style="margin: 0;">
          <li v-for="point in fjSellingPoints5" :key="point.value" class="selling_point">
            <label>
              <input type="checkbox" name="data.fj_selling_points4" @change="checkboxMaxSelection(2, 'data.fj_selling_points4')" v-model="data.fj_selling_points" :value="point">{{ point.label }}
            </label>
          </li>
        </ul>
      </td>
    ...
    <!-- <tr id="dispPosition" v-if="data.situation === 'public'">
      ...
      <td>
        <select v-model="data.orderNumber" class="w-25">
          ...
          <option v-for="(cast, index) in castData" :value="index + 1">
            {{ cast.castName }}の次
          </option>
        </select>
      </td>
  ...
  <div class="d-flex align-items-center" style="height: 130px;" v-if="isAnyFieldEmpty">
    <button type="button" class="btn btn-secondary w-25 h-50 mx-auto pe-none">追加する</button>
  </div>
  <div class="d-flex align-items-center" style="height: 130px;"  v-else>
    <button type="button" class="btn btn-success w-25 h-50 mx-auto" @click="openUploadModal">追加する</button>
  </div>
  <UploadModal
...
import { set, update, ref as dbRef, get, onValue, off } from "firebase/database";
import { getDownloadURL, ref as imgRef, uploadBytesResumable } from "firebase/storage";
...
// const castData = ref([]);
// const orders = ref({});
// const castNames_fetchData = async () => {
//   const castDataRef = dbRef(db, `users/${userStore.accountKey}/add_girl/cast_data`);
//   const snapshot = await get(castDataRef);
//   const childSnapshotData = [];
//   snapshot.forEach((childSnapshot) => {
//     const value = childSnapshot.val();
//     childSnapshotData.push({...value});
//   });
//   castData.value = childSnapshotData;
// }
...
//   castData.value.sort((a, b) => {
//     const orderA = orders.value[a.id] !== undefined ? orders.value[a.id] : Infinity;  // orders.value[a.id] !== undefined が真ならorders.value[a.id]、偽ならInfinityが入る
//     const orderB = orders.value[b.id] !== undefined ? orders.value[b.id] : Infinity;
//     if (orderA === orderB) {
//       return b.id - a.id;
//     }
...
// リアルタイムリスナー関数
const castNameChecker = () => {
  const castDataRef = dbRef(db, `users/${userStore.accountKey}/add_girl/cast_data`);
  off(castDataRef); // 既存リスナーを解除してリソースを節約
  onValue(castDataRef, (snapshot) => {
    const castData = snapshot.val();
    const AllCastNames = Object.keys(castData || {});
    isDuplicateName.value = AllCastNames.includes(data.castName);
  });
};
...
const isAnyFieldEmpty = computed(() => {
  ...
  return isFieldEmpty || isDuplicateName.value;
});

// data.castName の監視
watch(
  () => data.castName,  //  監視したいデータ
  (newValue) => { //  データが変わったらnewValueが呼ばれる
    if (newValue) {
      castNameChecker(); // 名前が変更されるたびにチェック
    }
  }
);

//  追加ボタンを押したら上までスクロールする
...
const add_girl = async()=> {
  ...
  await set(dbRef(db, `users/${userStore.accountKey}/logs/girls_log/${id}`), {
    content: '新規登録：' + data.castName,
    registration_date: dayjs().format('YYYY-MM-DD HH:mm:ss'),
  });

  //  公開の場合のみ並び順を登録する（htmlはv-ifで制御してるよ）
  if (data.situation === 'public') {
    if (ordersData) {
      const orderNumber = Object.keys(ordersData).length;
      updates[`users/${userStore.accountKey}/add_girl/orders/${id}`] = parseInt(orderNumber);
    } else {
      updates[`users/${userStore.accountKey}/add_girl/orders/${id}`] = 0; //  ordersDataがnull/undefinedなら0を代入
    }
  }
  ...
  Object.assign(updates, {
    ...
    [`${castProfilePath}/flags`]: data.flags.length > 0 ? data.flags : { 0: { label: '', value: '' } },
    ...
    [`${castProfilePath}/selling_points`]: data.selling_points.length > 0 ? data.selling_points : { 0: { label: '', value: '' } },
    [`${castProfilePath}/ch_selling_points`]: data.ch_selling_points.length > 0 ? data.ch_selling_points : { 0: { label: '', value: '' } },
    [`${castProfilePath}/ec_selling_points`]: data.ec_selling_points,  //  必須項目なので0個な事がない
    [`${castProfilePath}/pl_selling_points`]: data.pl_selling_points.length > 0 ? data.pl_selling_points : { 0: { label: '', value: '' } },
    [`${castProfilePath}/fj_selling_points`]: data.fj_selling_points.length > 0 ? data.fj_selling_points : { 0: { label: '', value: '' } },
...
const openUploadModal = async()=> {
  ...
  await add_girl();
  try {
    const response = await fetch(`${userStore.API_BASE_URL}/add-castData`, {
      method: 'POST',
      headers: {
